...
def create_email_content(posts: List[Dict[str, str]], total_count: int) -> str:
    """
    ...
    if posts:
        ...
        for idx, post in enumerate(posts, 1):
            ...
            post_type = post.get('content_type', 'Материал')  # Тип: статья, новость, видео и т.д.
            description = post.get('description', post.get('post_content', ''))[:350]  # Максимум 350 символов
            url = post.get('url', '#')
...
def send_email_notification(
    ...
    try:
        ...
        msg['Subject'] = f"{settings.email_subject} ({datetime.now().strftime('%d.%m.%Y %H:%M')})"

        # Создаем HTML содержимое
        html_content = create_email_content(posts, len(posts))
        ...
        logger.info(f"📧 Подключение к SMTP серверу: {settings.smtp_server}:{settings.smtp_port}")
        ...
        try:
            ...
            logger.error("💡 Проверьте SMTP_USERNAME и SMTP_PASSWORD (для Gmail используйте App Password)")
            return False
            ...
            logger.error("💡 Проверьте интернет-подключение и настройки брандмауэра")
            return False

        logger.success(f"📧 Email успешно отправлен!")
