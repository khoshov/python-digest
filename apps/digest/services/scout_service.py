...
class ScoutService:
    ...
    def search_google_news(self, query: str, api_key: str, cse_id: str, num_results: int = 5) -> List[Dict[str, str]]:
        """
        ...
        params = {
            ...
            "num": min(num_results, 10),    # Количество результатов (макс 10 для Google API)
        ...
        try:
            ...
            logger.debug(f"Google Search: найдено {len(filtered_results)} статей для запроса '{english_query}' (оригинал: '{query}')")
            return filtered_results
            
        except requests.exceptions.RequestException as e:
            logger.error(f"Ошибка при поиске через Google API для запроса '{english_query}': {e}")
            return []
        except Exception as e:
            logger.error(f"Неожиданная ошибка при поиске '{english_query}': {e}")
            return []

    def fetch_rss_headlines(self, feed_url: str, hours: int = 12) -> List[Dict[str, str]]:
        """
    ...
    def collect_insights(self, keywords: List[str], 
        ...
        if google_api and google_cse:
            ...
            for kw in keywords:
                ...
                items = self.search_google_news(kw, google_api, google_cse, max_per_source)
